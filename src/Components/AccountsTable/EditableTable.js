...
import {
    Button, FormControl, InputLabel, MenuItem, Select, Table, TableBody, TableCell, TableHead, TableRow, withStyles
...
const OurSelect = ({name, value, selectMessage, options, classes, tableName, ...props}) => {
    ...
    return (<FormControl className={classNames(classes.selectFormControl, `selectFormControl_${tableName}`)}>
        <InputLabel
            className={classNames(classes.selectInputLabel, `selectInputLabel_${tableName}`)}
            htmlFor={name}>
            {selectMessage}
        </InputLabel>
        <Select
...
const EditableRow = ({   fieldsArr = [],
    ...
    >
        {fieldsArr.map((item, i) => {
            return (<TableCell
                className={classNames(classes.tableBodyCell, `tableBodyCell_${tableName}`)}
                key={i}
            >
                {item.type === "select" ? (<OurSelect
                    tableName={tableName}
                    classes={{
                        ...selectClasses
                    }}
...
const Row = ({
                 data, handleEditRow, classes, tableName, handleDeleteRow, isAdding, isEditing
             }) => {
    return (<TableRow
        className={classNames(classes.tableBodyRow, `tableBodyRow_${tableName}`)}
    >
...
class EditableTable extends React.Component {
    state = {
        allRowsData: (this.props.defaultData || []).map(item => ({
            isEditing: false, rowData: item
        })), isAdding: false, isEditing: false, editingIndex: null
    };
    handleSave = row => {
        if (this.state.isEditing) {
            ...
            this.setState({
                allRowsData: [...this.state.allRowsData, {isEditing: false, rowData: row}], isAdding: false
            }, this.setToParent);
        }
    };
    setToParent = () => {
    ...
    handleCancel = index => {
        if (this.state.isEditing) {
            const arr = this.state.allRowsData.map((item, i) => {
                if (i === index) {
                    return {
                        isEditing: false, rowData: item.rowData
                    };
                } else return item;
            });
    ...
    handleEditRow = index => {
        const arr = this.state.allRowsData.map((item, i) => {
            if (i === index) {
                return {
                    isEditing: true, rowData: item.rowData
                };
            } else return item;
        });
    ...
    render() {
        const {
            fieldsArr = [], classes = {}, tableName, addRowBtnText, initWithoutHead
        } = this.props;
        const {allRowsData = [], isAdding, editingIndex, isEditing} = this.state;
        let headRow = [...fieldsArr.map(item => ({label: item.label, name: item.name})), {
            label: "Actions",
            name: "actions"
        }];
        ...
        return (<>
            <Table className={classNames(classes.table, `table_${tableName}`)}>
                {showHeader && (<TableHead className={classNames(classes.tableHead)}>
                    <TableRow
                        className={classNames(classes.tableHeadRow, `tableHeadRow_${tableName}`)}
                    >
                        {headRow.map(({label, name}, i) => (<TableCell
                            className={classNames(classes.tableHeadCell, classes[`tableHeadCell${name}`], `tableHeadCell_${tableName} tableHeadCell_${name}`)}
                            key={i}
                        >
                            {label}
                        </TableCell>))}
                ...
                >
                    ...
                    {isAdding && (<EditableRow
                        ...
                        inputClasses={{
                            inputWrapperDiv: classes.inputWrapperDiv, input: classes.input, error: classes.error
                        }}
                        classes={{
                            tableBodyRow: classes.tableBodyRow,
                            tableBodyCell: classes.tableBodyCell,
