...
import {registerUser, registerUserFailure, registerUserSuccess} from "../../../redux/actionCreators";
import {connect} from "react-redux";

const registerUserPromise = (registerUser, values, setLoading) => (new Promise(async (resolve) => {
    setLoading(true)
    console.log('start awaiting on view')
    await registerUser(values)
    resolve();
}))

const RegistrationForm = ({
                              isOpen, setRegistrationIsOpen, registerUser, ...props
                          }) => {
    ...
    const handleSubmit = async (values) => {
        await registerUserPromise(registerUser, values, setLoading).then(_ => {
            console.log('stop awaiting on view')
            setLoading(false)
            console.log('isSuccess? ', props.registration.isSuccess)
            console.log('errors: ', props.registration.errors)
    ...
    const formik = useFormik({
        ...
        }, validationSchema: SignupSchema, onSubmit: async (values) => {
            await handleSubmit(values);
        },
    });

    return isOpen ? (<div className={classes.wrapper}>
            <div
